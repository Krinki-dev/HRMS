...
router.get('/stats', async (req, res) => {
  try {
    ...
    const [allTenants, recentTenants, totalEmployees, kycCount] = await Promise.all([
      c.$queryRaw`
        SELECT
          COUNT(*)                                                    AS total,
      ...
      c.$queryRaw`
        ...
        LIMIT 10
      `,
      c.$queryRaw`SELECT SUM(max_employees) as total FROM tenants WHERE deleted_at IS NULL`,
      c.$queryRaw`SELECT COUNT(*) as total FROM kyc_cache`.catch(() => [{ total: 0 }])
    ]);

    const stats = allTenants[0] || {};

    return sendSuccess(res, {
      totalTenants:  Number(stats.total || 0),
      activeSubs:    Number(stats.active || 0),
      totalEmployees: Number(totalEmployees[0]?.total || 0),
      kycRecords:    Number(kycCount[0]?.total || 0),
      
      total:         Number(stats.total || 0), // backward compat
      active:        Number(stats.active || 0), // backward compat
...
/**
 * GET /plans
 * Returns the dynamic configuration object needed by PricingManager.
 */
router.get('/plans', async (req, res) => {
  try {
    const c = getCentral();
    const row = await c.$queryRaw`SELECT values FROM platform_settings WHERE id = 'Subscription Plans' LIMIT 1`;
    
    const defaults = { 
      plans: [
        { id: 'starter', name: 'Starter', price: 2999, maxEmployees: 100, features: ['Core HR', 'Payroll'] },
        { id: 'pro', name: 'Professional', price: 7999, maxEmployees: null, features: ['Advanced HR', 'Recruitment'] }
      ], 
      modulePricing: {},
      discounts: { tenure: [], bundle: {} },
      promoCodes: [],
      gstRate: 18 
    };

    if (row.length && row[0].values) {
      return sendSuccess(res, { ...defaults, ...row[0].values });
    }
...
/**
 * GET /plans
 * Returns the dynamic platform plan catalog based on market research and the pricing guide.
 */
router.get('/plans', async (req, res) => {
  const plans = [
    { 
      ...
      baseEmployees: 25,
      expansionPrice: 50,
      features: ['Core HR Foundation', 'Attendance & Leave', 'Standard Support'],
      modules: [
        { id: 'payroll', name: 'Payroll', price: 30, type: 'per_employee', desc: 'Auto-filing & TDS' },
        { id: 'compliance', name: 'Compliance', price: 18, type: 'per_employee', desc: 'PF/ESI/PT/LWF' },
        { id: 'field_force', name: 'Field Force', price: 25, type: 'per_employee', desc: 'GPS Tracking' }
      ]
    },
    { 
      id: 'pro', 
      ...
      baseEmployees: 100,
      expansionPrice: 40,
      features: ['Everything in Starter', 'Recruitment ATS', 'Priority Support'],
      modules: [
        { id: 'payroll', name: 'Payroll', price: 25, type: 'per_employee' },
        { id: 'recruitment', name: 'Recruitment', price: 1200, type: 'flat', desc: 'ATS & Job Boards' },
        { id: 'performance', name: 'Performance', price: 900, type: 'flat', desc: 'KRA/KPI Appraisal' }
      ]
    },
    { 
      id: 'enterprise', 
      ...
      price: null, 
      period: null, 
      features: ['No Employee Limits', 'Own Server Deployment', 'Dedicated Account Manager'],
      note: 'Per-employee hosting fees are waived for "Own Cloud" or "On-Premise" models.'
    }
  ];
  return sendSuccess(res, { plans, gstRate: 18 });
});
